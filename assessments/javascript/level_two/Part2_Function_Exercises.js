...
function luckySum(a, b, c){
  ...
  for (let i = 0; i < numsArr.length; i++){
    if (numsArr[i] !== 13) finalVal += numsArr[i]
    else break
  }
...
function caught_speeding(speed, is_birthday){
  function isBirthday(speed){
    if (speed <= 65) return 0
    else if (speed <= 85) return 1
    else return 2
  }

  function notBirthday(speed){
    if (speed <= 60) return 0
    else if (speed <= 80) return 1
    else return 2
  }

  if (is_birthday) return isBirthday(speed)
  else return notBirthday(speed)
}
